...
\tofix{Sutton854066 is missing from the bibtex file}
...
Given the industry-oriented nature of startups and the limited number of formally-published literature in this area, we conducted a Multivocal Literature Review (MLR)~\cite{GAROUSI2019101}.\serge{Don't use acronsyms -- replace MLR with ``literature review''. I already started doing this.}
...
\tofix{The structure of the paper isn't clear from this overview.
Where do you address the reserch questions? In each section? Towards the end?
You mention a framwrok, taxonomy as the result: where does this come?}
...
\tofix{The picture and the process described here are not really consistent.
Minor issue right now but they must be sycnhronised.}
This search was conducted in October 2023 and the analysis and reporting was completed in December 2023.\tofix{Update these dates.}
...
\begin{figure}
  ...
  \tofix{Remove the acromym MLR from the picture}
 \label{fig:MLRprocess}
\end{figure}

\subsection{Conducting the Multi-Vocal Literature Survey}
...
We will~\tofix{No future or pas tense in a paper. For the reader this is all in present tense.}
...
\serge{I know this pops up in every literature survey.
But I always have trouble with such an observation.
The number of academic papers is growing exponentially.
And if you include grey literature it probably grows super exponentially.
So of course, for any given topic the number of sources will grow.}
...
Figure \ref{fig:Sources} shows the categories of our primary sources.
...
\serge{I have a problem with this ``clearly shows \ldots''; you search is biased.
I would rather see ``confirms''.}
...
\serge{The last section of the intro states that you would address the research questions in Section~\ref{Sec:Discussion}.
Why then do you do it here?}

\subsection{Key Dimensions and Types of Technical Debt}
\tofix{Are these dimensions part of the State of the art or is this actually a new section?}
...
child [concept color=lightgray] { node {Classification}
